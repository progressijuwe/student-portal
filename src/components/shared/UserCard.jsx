...
export default function UserCard({ user, onView, onEdit, onDelete }) {
    const { id, name, email, phone, department, level, enrollmentYear, joinYear, courses } = user
    ...
    return (
        ...
        >
            <header className="flex flex-col gap-1">
                <h3 id={`user-${id}`} className="font-medium text-brand-orange text-sm">
                {name}
                </h3>
            ...
            <ul className="text-xs flex flex-col gap-3">
                <DetailItem icon={EmailIcon} label="Email">{email}</DetailItem>
                <DetailItem icon={PhoneIcon} label="Phone">{phone}</DetailItem>
                <DetailItem icon={DeptIcon} label="Department">
                    {department}{levelOrCourseLoad && ` | ${levelOrCourseLoad} ${level ? 'level' : 'courses'}`}
                </DetailItem>
                <DetailItem icon={CalendarIcon} label="Year" iconClass="[&_path]:fill-label">
                    {year}
                </DetailItem>
            </ul>

            <div className="flex gap-2">
                <ActionButton onClick={() => onView?.(user)} label={`View ${name}`}>View</ActionButton>
                <ActionButton onClick={() => onEdit?.(user)} label={`Edit ${name}`}>Edit</ActionButton>
                <ActionButton onClick={() => onDelete?.(user)} variant="danger" label={`Delete ${name}`}>Delete</ActionButton>
            </div>
        </article>
    )
}
...
function ActionButton({ children, onClick, variant = "default", label }) {
    const base = "px-3 py-1 rounded text-xs border transition focus:outline-none focus:ring-2 focus:ring-offset-1"
    const styles = {
    ...
    return (
        <button type="button" onClick={onClick} aria-label={label} className={`${base} ${styles[variant]}`}>
        {children}
        </button>
    )
}