	...
	return (
		<tr className='border-t border-border text-sm'>
			...
			<td className='py-4 px-2'>
				{/* Lecturers carry a titled displayName ("Dr. Ada Nwosu");
				    students only have a plain name. */}
			...
			<td className='py-4 px-2 text-label font-medium'>
				<span className='flex items-center gap-1'>
					<EmailIcon className='size-4' /> {user.email}
				</span>
				<span className='flex items-center gap-1'>
			...
			<td className='py-4 px-2'>
				<div className='flex justify-center gap-2.5'>
					<button aria-label='View' onClick={() => onView?.(user)}>
						<Eye className='size-5 [&_path]:stroke-label' />
					</button>
					<button aria-label='Edit' onClick={() => onEdit?.(user)}>
