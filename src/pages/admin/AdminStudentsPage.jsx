        ...
        <EntityPageShell title="Students">
            ...
            <AnimatePresence>
                {modal.type === MODAL.ADD && (
                    <AddStudentModal onClose={close} onSuccess={() => handleSuccess(MODAL.SUCCESS)} />
                )}
                {modal.type === MODAL.SUCCESS && (
                    <AddSuccessModal onClose={close} text="Student Added Successfully" />
                )}
                {modal.type === MODAL.FILTER && (
                    <FilterModal heading="Filter Students" fields={studentFilterFields} onClose={close} onApply={setFilters} initialFilters={filters} />
                )}
                {modal.type === MODAL.DELETE && (
                    <DeleteStudentModal student={modal.data} onClose={close} onSuccess={() => handleSuccess(MODAL.DELETE_SUCCESS)} />
                )}
                {modal.type === MODAL.DELETE_SUCCESS && (
                    <AddSuccessModal onClose={close} text="Student Deleted Successfully" />
                )}
                {modal.type === MODAL.EDIT && (
                    <UpdateStudentModal student={modal.data} onClose={close} onSuccess={() => handleSuccess(MODAL.EDIT_SUCCESS)} />
                )}
                {modal.type === MODAL.EDIT_SUCCESS && (
                    <AddSuccessModal onClose={close} text="Student Updated Successfully" />
                )}
                {modal.type === MODAL.VIEW && (
                    <ProfileModal heading="Student Profile" user={modal.data} userType="lecturer" onClose={close} onEdit={handleEdit} onDelete={handleDelete} />
                )}
            </AnimatePresence>
        </EntityPageShell>
    )
