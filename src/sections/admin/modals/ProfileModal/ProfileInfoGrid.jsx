...
const INFO_CONFIG = {
    email: { Icon: Email, bg: "bg-brand-blue", iconColor: "text-brand-blue-border" },
...
function Info({ Icon, label, children, bg = "bg-gray-100", iconColor = "text-black" }) {
    return (
        <div className="flex gap-4 items-start">
            {Icon && (
                <span className={`p-2 rounded-lg ${bg}`}>
                    <Icon className={`size-5 ${iconColor} [&_path]:stroke-current [&_path]:stroke-0.5`} />
                </span>
            )}

            <div className="flex flex-col text-sm">
                <span className="text-label text-xs">{label}</span>
                <span className="font-medium text-sm text-black">{children}</span>
            </div>
        </div>
    )
}