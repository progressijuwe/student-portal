...
export default function CoursePane({ course }) {
	if (!course) return null;

	const details = [
		{ label: 'level', value: `${course.level} Lvl` },
		{ label: 'Credit Units', value: course.credits },
		{
			label: 'Semester',
			value: course.semester === 'first' ? 'First' : 'Second',
		},
		{ label: 'Enrolled Students', value: course.enrolledCount },
	];
	return (
			...
			<div className='grid grid-cols-4 border-t border-[#E5E7EB] py-4'>
				{details.map((det) => (
					<div key={det.label} className='flex flex-col w-fit'>
						<span className='text-[10px] md:text-sm text-label leading-5 text-nowrap capitalize'>
							{det.label}
						</span>
						<p className='text-xs md:text-base font-medium text-[#101828]'>
							{det.value}
						</p>
					</div>
				))}
