...
export default function ProfileDownloads({ role }) {
	const downloads =
		role === 'lecturer' ? lecturerDownloads : studentDownloads;

	return (
		<div className='flex flex-col gap-3 lg:max-w-81 w-full'>
			<span className='flex gap-2.5'>
				<Files className='size-5 lg:size-6' />
				<h4 className='text-sm lg:text-xl font-semibold text-black'>
					Quick Downloads
				</h4>
			</span>
						...
						<div className='flex gap-2.5 items-center'>
							...
							<div>
								...
								<span className='flex gap-1 text-xs text-label font-medium'>
									...
									<p>
										{record.lastUpdated ?? record.semester}
									</p>
								</span>
							</div>
						</div>
